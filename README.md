...
- 상단 **지적편집도** 버튼으로 카카오 지적편집도 레이어를 켜고 끌 수 있습니다. 참고용이며 현행 지적 정보와 일치하지 않을 수 있습니다.
...
- 이전 저장 대비 추가·수정한 글은 초록색으로 강조하고, 삭제·교체된 기존 글은 이전 메모 원문 전체와 줄바꿈을 유지하고 해당 부분에만 붉은 취소선으로 표시합니다. 같은 날짜의 여러 저장도 각각 보존됩니다.
...
기록 상세의 **이미지 · 파일 첨부**에서 여러 파일을 선택하면 즉시 저장됩니다.
새 지도 기록을 만들면 상세 화면이 열리므로 이어서 첨부할 수 있습니다.
파일당 최대 10MB, 기록당 최대 20개입니다. PNG/JPEG/GIF/WebP는 미리보기와 크게 보기를 지원하며 그 외 파일은 다운로드로 확인합니다.
첨부 날짜와 작성 아이디를 표시하고, 기록 작성자만 업로드·삭제할 수 있습니다.
조회는 기존 기록과 같은 로그인 사용자 공유 범위를 따릅니다.

파일 원본은 PostgreSQL attachments 테이블에 저장됩니다. 로컬 소스 파일이나 공개 GitHub 저장소에 첨부파일을 저장하지 않습니다.
DB 백업에 첨부파일도 포함해야 하며 파일 사용량만큼 DB 용량이 증가합니다.
메모를 수정해도 첨부는 유지되며, 첨부를 삭제하거나 기록 전체를 삭제하면 원본도 함께 삭제됩니다. 첨부파일 자체의 버전 이력은 저장하지 않습니다.

두 기록 API 각각에서:
- GET /{id}/attachments: 목록
- POST /{id}/attachments: multipart file 업로드
- GET /{id}/attachments/{attachment_id}: 다운로드 (?preview=true는 지원 이미지 미리보기)
- DELETE /{id}/attachments/{attachment_id}: 첨부 삭제

검증: uv run --directory backend python tests/attachments.py

## 건물·토지 공공자료

기록 상세의 **건물 · 토지 공공자료**에서 지번 검색 또는 핀 위치로 대상 필지를 선택합니다.
다각형 구역은 여러 필지를 포함할 수 있어 조회할 지번을 직접 선택합니다. 필지번호(PNU) 직접 입력도 가능합니다.

- 토지이용계획: 브이월드 용도지역·지구 및 저촉 정보 (공식 확인서 원본 발급 아님)
- 건축인허가: 건축HUB 기본개요
- 주택인허가: 건축HUB 기본개요
- 건축물대장: 건축HUB 표제부

작성자는 각 서비스의 조회·저장 버튼으로 자료를 DB에 저장할 수 있습니다. 출처, 조회 시각, 조회자, 지번과 함께 보관하며,
같은 기록·필지·서비스를 재조회하면 저장 자료를 갱신합니다. 이전 조회의 버전 이력은 보관하지 않습니다.
다른 필지를 조회하면 별도로 보관합니다. 로그인 사용자의 열람 범위는 기존 기록과 같으며, 기록 삭제 시 함께 삭제됩니다.
조회 실패는 기존 저장 자료를 변경하지 않습니다. 최대 1,000건까지 가져오며 초과 자료는 일부 저장임을 화면에 표시합니다.

인증키는 서버의 backend/.env에만 입력합니다. 입력 위치와 실행 방법은 [PUBLIC_DATA_SETUP.md](PUBLIC_DATA_SETUP.md)를 확인하세요.
인증키가 없는 상태에서는 미설정 안내를 보여 주며 저장된 자료는 계속 열람할 수 있습니다.
지적편집도는 지도 표시 기능으로, 토지이용계획 속성 조회와 별도입니다.

추가 검증:
- uv run --directory backend python tests/public_data.py
- uv run --directory backend python tests/attachments.py
