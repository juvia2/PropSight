  ...
  return <div className="app">
    ...
    <main>
      ...
      <div className="dashboard"><aside>
        ...
        <div className="record-list">{visible.length ? visible.map(item => <button className="record" key={`${item.resource}-${item.id}`} onClick={() => focus(item)}><span className="record-icon" style={{ color: COLORS[item.properties.category] }}>{item.geometry.type === 'Point' ? '⌖' : '⬡'}</span><span><small style={{ color: COLORS[item.properties.category] }}>{item.properties.category}</small><strong>{item.properties.name}</strong><p>{item.properties.memo || '메모 없음'}</p><small className="record-author">{item.properties.team_name} · {item.properties.author_name} {item.properties.author_username ? `(@${item.properties.author_username})` : ''}</small></span><span>›</span></button>) : <div className="empty"><span>⌖</span><p>{items.length ? '표시할 기록이 없습니다.' : '아직 저장한 장소가 없습니다.'}</p><small>핀과 구역으로 첫 현장 기록을 남겨보세요.</small></div>}</div></div>
