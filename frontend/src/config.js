...
export const SANDBOX_TEAM_NAMES = ['severcheckup', 'servercheckup', 'checkup', 'server', '서버확인']
export const isSandboxTeamName = value => {
  const normalized = value.toLowerCase().replace(/[\s_-]/g, '').replace(/팀$/, '')
  return SANDBOX_TEAM_NAMES.includes(normalized) || normalized.includes('checkup')
}
